...
final class NumberConverter {
    ...
    public static String format(double amount, ExpansionsConfig.NumberFormat mode, List<String> suffixes, boolean allowDecimals) {
        return switch (mode) {
    ...
    private static String formatShort(double amount, List<String> suffixes, boolean allowDecimals) {
        ...
        int exponent = 0;
        while (exponent < suffixes.size() - 1 && reduced >= 1000) {
            reduced /= 1000;
            exponent++;
        }
    ...
    public static OptionalDouble parse(String raw, boolean allowShorthand, List<String> suffixes) {
